      ...
      {filtered.map((lot) => {
        ...
        return (
          <Card key={lot.id} className="p-0 overflow-hidden">
            {/* GRID en vez de flex para que NUNCA empuje el ancho del móvil */}
            <div className="p-4 grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
              <div className="min-w-0">
                ...
                {(lot as any).notes ? (
                  <div className="text-xs text-gray-500 mt-2 line-clamp-2 break-words">
                    {(lot as any).notes}
                  </div>
                ) : null}
              </div>

              {/* Ojo: sin shrink-0, con min-w-0 y max-w-full */}
              <div className="min-w-0 max-w-full sm:text-right sm:justify-self-end">
                <div className="text-sm font-semibold whitespace-nowrap">
                  {formatCurrency(Number(lot.total_cost ?? 0))}
                </div>

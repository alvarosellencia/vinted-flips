  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        {statusFilters.map((s) => (
          <button key={s.key} onClick={() => setFilter(s.key)} className={chip(filter === s.key)}>
            {s.label}
          </button>
        ))}
      </div>
        ...
        return (
          <Card key={it.id} className="p-0 overflow-hidden">
            <div className="p-4 space-y-3">
              {/* Cabecera: grid para que chip no rompa el ancho */}
              ...
              {/* KPIs: grid minmax para que no desborde */}
              <div className="grid grid-cols-3 gap-2 text-xs min-w-0">
                <div className="min-w-0">
                  <div className="text-gray-500">Coste</div>
                  <div className="font-medium whitespace-nowrap">{formatCurrency(cost)}</div>
                </div>
                <div className="min-w-0">
                  <div className="text-gray-500">Venta</div>
                  <div className="font-medium whitespace-nowrap">{it.sale_price ? formatCurrency(Number(it.sale_price)) : '—'}</div>
                </div>
                <div className="min-w-0">
